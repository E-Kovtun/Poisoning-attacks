...
def freeze_weights(clean_checkpoint, freeze_part, data_config, model_name, device):
    ...
    if freeze_part == "freeze_emb":
        for param in poisoned_net.cat_embedding.parameters():
            param.requires_grad = False
    if freeze_part == "freeze_emb_enc":
        for param in poisoned_net.cat_embedding.parameters():
            param.requires_grad = False   
        if (model_name == "lstm") or (model_name == "lstmatt"):
            for param in poisoned_net.lstm.parameters():
                param.requires_grad = False  
        if model_name == "cnn":
            for param in poisoned_net.convs.parameters():
                param.requires_grad = False   
        if model_name == "transformer":
            for param in poisoned_net.transformer_encoder.parameters():
                param.requires_grad = False          
    if freeze_part == "freeze_linear":
        for param in poisoned_net.linear_layer.parameters():
            param.requires_grad = False             
        ...
        for model_name in model_names:
                for freeze_part in freeze_parts:
                    for i in num_launches:    
                        ...
                        checkpoint_dma_folder = os.path.join(checkpoints_folder, data_name, model_name, attack_name, freeze_part)
                        ...
                        clean_net, net = freeze_weights(clean_checkpoint, freeze_part, data_config, model_name, device)
                        ...
                        for epoch in range(1, 500):
                            ...
                            for batch_tuple, batch_tuple_clean in tqdm(zip(poisoned_train_dataloader, train_dataloader), total=len(poisoned_train_dataloader)):
                                ...
                                loss = loss_func(output, batch_target) + loss_sim(output_on_clean[:, 1], output_clean[:, 1])
                        ...
                        result_dma_folder = os.path.join(results_folder, data_name, model_name, attack_name, freeze_part)
